...
# function for channel stats using channel ids
def get_channel_stats(youtube, channel_ids):
    ...
    try:
        request = youtube.channels().list(
            ...
            #To concatenate multiple ids separated by comma.
            id=",".join(channel_ids)
        )
    ...
    # iterate through items of channels.
...
#function to get all videos detail with video ids
...
def get_all_video_stats(channel_stats):
    ...
    video_stats_df = pd.DataFrame(columns=[
                                  'Title', 'Published_date', 'Views', 'Likes', 'Dislikes', 'Comments', 'channel_name', 'playlist_id'])

    for item in channel_stats:
        video_ids = get_video_ids(youtube, item['playlist_id'])
        video_stats = get_video_details(youtube, video_ids)

        df2 = pd.DataFrame(video_stats, columns=[
                           'Title', 'Published_date', 'Views', 'Likes', 'Dislikes', 'Comments', 'channel_name', 'playlist_id'])
...
save_barplot('channel_name', 'subscribers',
             channel_stats_df, 'subscribers_by_channel.png')
save_barplot('channel_name', 'number_of_videos',
             channel_stats_df, 'videos_by_channel.png')

# Block to fetch and plot video stats data
all_video_stats = get_all_video_stats(channel_stats)
print(all_video_stats)
top_10_videos = all_video_stats.sort_values(
